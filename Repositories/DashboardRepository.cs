...
public class DashboardRepository(AppDbContext db) : IDashboardRepository
{
    ...
    {
        var total = await db.AppOpensDaily.SumAsync(v => (int?)v.OpenCount, ct) ?? 0;
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
        var weekStart = today.AddDays(-6);

        var todayCount = await db.AppOpensDaily
            .Where(v => v.ViewDate == today)
            .SumAsync(v => (int?)v.OpenCount, ct) ?? 0;

        var weekCount = await db.AppOpensDaily
