...
3. Gemini عنوان و توضیح فارسی و انگلیسی، متن فارسی قصه و توضیح انگلیسی کاور را برمی‌گرداند.
4. پس از دریافت قصه، تولید کاور از توضیح متنی و تولید روایت صوتی از متن قصه هم‌زمان آغاز می‌شوند.
5. کاور تولیدشده، یا خود نقاشی در حالت جایگزین، بارگذاری می‌شود. آدرس‌ها و محتوای قصه ذخیره و وضعیت پیش‌نویس `ready` می‌شود.
...
- بدنه شامل تنها فیلد `drawing` از نوع فایل، همراه نام فایل است. اپ آن را با `FormData` می‌سازد و boundary توسط کلاینت HTTP تعیین می‌شود.
...
```http
POST {BaseUrl}/v1beta/models/{Model}:generateContent?key=<URL-encoded-API-key>
...
عنوان و توضیح دوزبانه‌اند؛ خود `storyScript` فقط فارسی است. سن، تعداد کلمات، لحن و نبود اعراب در این مرحله دستور متنی به مدل هستند، نه پارامترهای مستقل API یا تضمین اعتبارسنجی خروجی.
...
```http
POST {BaseUrl}/v1beta/models/{CoverImageModel}:generateContent?key=<URL-encoded-API-key>
Content-Type: application/json; charset=utf-8
Accept: application/json
```
...
- آدرس پایه پیش‌فرض: `https://generativelanguage.googleapis.com`.
...
اولویت به اولین مقدار غیر null است؛ کنترل مقادیر خالی در کلاینت‌ها نیز وجود دارد. مقدار نهایی محیط سرور زنده در این بررسی خوانده نشده است. پوشه `deploy/ai-proxy` مسیر `/gemini` را به سرویس Google هدایت می‌کند؛ استفاده واقعی از آن وابسته به تنظیم `BaseUrl` است. هیچ کلید واقعی در این سند ثبت نمی‌شود.
